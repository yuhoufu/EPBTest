...
function Refresh-FixtureIdentity([string]$Path) {
    ...
    $components = @($files | Where-Object { $_.name -match '\.(exe|dll)$' } | ForEach-Object { @{name=$_.name;fileVersion='2.17.1.0'} })
    ...
    @{ gitCommit=('a'*40); packageContentSha256=(Get-DeploymentAggregateHash $map); files=$files; componentIdentities=$components;
        fileVersion='2.17.1.0'; recoveryArchitectureGeneration='EPB-V2.17'; watchdogSchema=7;
