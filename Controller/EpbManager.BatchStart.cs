...
namespace Controller
{
    ...
    public partial class EpbManager
    {
        ...
        private readonly ConcurrentDictionary<string, FormalSafetyClosureTransaction>
            _formalSafetyClosureTransactions =
                new ConcurrentDictionary<string, FormalSafetyClosureTransaction>();
            ...
            bool persistenceRequired)
        {
            ...
            var runEpoch = Interlocked.Read(ref _runEpoch);
            var key = $"{runId:N}:{runEpoch}:{formalSlot}:{channel}";
            var transaction = _formalSafetyClosureTransactions.GetOrAdd(
                key,
                _ => new FormalSafetyClosureTransaction());
            ...
            string terminalStatus = null, string terminalReason = null)
        {
            ...
            var closurePrefix = _activeBatchId.ToString("N") + ":";
            foreach (var key in _formalSafetyClosureTransactions.Keys
                         .Where(key => key.StartsWith(
                             closurePrefix,
                             StringComparison.Ordinal))
                         .ToArray())
                _formalSafetyClosureTransactions.TryRemove(key, out _);
