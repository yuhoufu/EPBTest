...
namespace Controller
{
    internal sealed class HydraulicGroupCoordinator
    {
        ...
        private readonly Controller.HydraulicController _hydCtl;

        // hydId 对应的一轮“闩锁”状态
        private sealed class Latch
        {
            public readonly object Gate = new();
            public HashSet<int> InFlight = new();        // 仍未到“电压释放点”的通道
            public bool PressureOn = false;              // 是否已进入“建压保持”状态
        ...
        public HydraulicGroupCoordinator(TestConfig test,
                                         ...
                                         Controller.HydraulicController hydCtl,
                                         IAppLogger log)
        {
        ...
        public async Task EnterElectricalPhaseAsync(int epbChannel, CancellationToken token)
        {
            ...
            lock (latch.Gate) latch.InFlight.Add(epbChannel);

            bool needBuild;
            lock (latch.Gate) needBuild = !latch.PressureOn;
            if (!needBuild) return;

            try
            {
                lock (latch.Gate) latch.PressureOn = true;
                ...
                if (_hydCtl != null)
                {
                    ...
                    {
                        try { _hydCtl.Release(hydId); } catch { /* 忽略异常 */ }
                ...
                else
                {
                    ...
                    if (_ao == null || _readPressure == null)
                    {
                        ...
                        lock (latch.Gate) latch.PressureOn = false;
                        return;
                    }
                    ...
                    latch.ReleaseActionAsync = async () =>
                    {
                        ...
                        try { var dev = hydId == 1 ? "Cylinder1" : "Cylinder2"; _ao.WritePercent(dev, 0); } catch { }
                    };
        ...
        private async Task FallbackHoldLoopAsync(int hydId, CancellationToken token)
        {
            ...
            {
                try { await Task.Delay(Math.Max(0, item.DurationMs), token); }
                catch (OperationCanceledException) { return; }
            }
