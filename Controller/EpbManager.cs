    ...
    public sealed partial class EpbManager
    {
        ...
        private void TryFinalizeCurrentCycleAfterSnapshot(int channel, bool hasSnapshotFiles)
        {
            ...
            catch (Exception ex)
            {
                ...
                _log.Warn(
                    $"EPB[{channel}] 报警圈封存边界异常，已降级为failed。" +
                    $"Cycle={cycleNumber} Error={ex.Message}",
                    "落盘");
            }
        }
        ...
        public async Task StartChannelAsync(int channel, CancellationToken uiToken = default)
        {
            ...
            // 单通道启动的DAQ自愈可能持续重试；提前建立停止令牌，使“停止”按钮随时可取消。
            ...
            if (learnCycles > 0)
            {
                ...
                try
                {
                    // 单卡钳启动与批量启动共用相同语义：软件失败持续自愈，只有本次
                    // 实时确认的控制/硬件故障才退出；旧状态不得直接挡住重新开始。
            ...
            _ = timer.StartAsync(null, staggerMs, async (i, token) =>
            {
                ...
                if (recorder != null)
                {
                    ...
                    catch (Exception ex)
                    {
                        if (controlSucceeded)
                            ...
                            _log.Warn(
                                $"EPB[{channel}] 失败圈封存异常 Cycle={i}: {ex.Message}",
                                "落盘");
                    }
                }
        ...
        private void OnDaqPersistenceStateChanged(DaqPersistenceStateChanged update)
        {
            ...
            {
                _ = TryCompleteDaqAutoRecoveryAsync(update.Device);
            }
            else if (update.State == DaqPersistenceState.Failed)
            {
                if (string.Equals(
                        ...
                        StringComparison.OrdinalIgnoreCase))
                    _ = HandleActiveCycleDataLimitExceededAsync(update);
                else
                    _ = BeginAndEscalateDaqPersistenceFailureAsync(update);
            }
            ...
            int recoverableAlarmChannel = 0)
        {
            ...
            try
            {
                ...
                var affectedCycles = affected.ToDictionary(
                    channel => channel,
                ...
                foreach (var channel in affected)
                {
                    ...
                    var cycle = affectedCycles[channel];
                    if (cycle != 0)
                        DiscardCurrentCycleForSoftwareRecovery(
                            channel,
                            context.CutoffUtc,
                ...
                if (restartDaq)
                {
                    ...
                    _ = ExportDaqIncidentSnapshotAsync(context, result.FailureReason, "20-rebuild");
                    if (!result.Recovered)
                    {
                        await EscalateDaqAutoRecoveryAsync(
                                device,
            ...
            DaqRecoveryResult recoveryResult = null)
        {
            ...
            // Independent DAQ groups are already safely de-energized. Software failures use
            // bounded backoff only inside the 60-second group deadline; when that deadline is
            // reached the affected hydraulic group is cleared and either restarted or isolated.
                ...
                try
                {
                    ...
                    while (true)
                    {
                        ...
                        catch (HydraulicReleaseTimeoutException ex)
                        {
                            ...
                            OnHydraulicFaultRaised(new ControlFault(
                                "HydraulicReleaseTimeout",
                                ex.Message,
                                FaultScope.HydraulicGroup,
                                channels,
                                hydraulicId,
                                DateTime.UtcNow,
                                fault.CorrelationId == Guid.Empty ? Guid.NewGuid() : fault.CorrelationId,
                                FaultClassification.HardwareConfirmed));
                            return;
                        }
                        catch (Exception ex)
                ...
                try
                {
                    ...
                    foreach (var channel in channels)
                    {
                        ...
                        PublishChannelRuntimeState(
                            ...
                            ChannelRuntimeState.Recovering,
                            "PowerSupplySelfHealing",
                            "程控电源通信/状态软件自愈中；当前圈已作废，其他电源组继续运行。",
                            affectedChannels: channels,
                    ...
                    while (true)
                    {
                        ...
                        {
                            var snapshot = _powerSupply.GetLatestSnapshot(groupId);
                            if (snapshot?.ProtectionTripped == true)
                            {
                                _powerSoftwareRecoveryGroups.TryRemove(groupId, out _);
                                OnPowerSupplyFaultRaised(new PowerSupplyFault
                                {
                                    TimestampUtc = DateTime.UtcNow,
                                    SupplyId = sourceFault.SupplyId,
                                    ElectricalGroupId = groupId,
                                    Code = "ProtectionTrip",
                                    Reason = "自愈实时预检确认程控电源保护仍处于触发状态。",
                                    AffectedChannels = channels,
                                    Snapshot = snapshot,
                                    Classification = FaultClassification.HardwareConfirmed
                                });
        ...
        private async Task<StopSafetyResult> RunStopSafetyAsync(StopContext context, CancellationToken token)
        {
            ...
            var persistenceDrained = await _persistence.DrainAsync(10000).ConfigureAwait(false);
            if (!persistenceDrained)
                _log.Warn("停止流程等待DAQ持久化队列10秒仍未排空，将由关闭流程取消并归还缓冲区。", "落盘");
                    ...
                    async (ch, ct) =>
                    {
                        ...
                        while (true)
                        {
                            ...
                            catch (Exception ex)
                            {
                                ...
                                if (!offSucceeded)
                                {
                                    var offFailure = new StartupPositioningResult
                                    {
                                        Channel = ch,
                                        Succeeded = false,
                                        Stage = StartupPositioningStage.None,
                                        Code = "StartupPositioningOutputOffCommandFailed",
                                        Reason =
                                            "OutputOffCommandFailed：启动定位软件异常后无法执行安全断电。" +
                                            $" Original={ex.GetType().Name}: {ex.Message}"
                                    };
                                    failedResults.Add(offFailure);
                                    await PublishStartupPositioningFailureAsync(offFailure)
                                        .ConfigureAwait(false);
                                    break;
                                }
                                ...
                                PublishChannelRuntimeState(
                                    ...
                                    $"启动定位软件异常，第{attempt}次自愈，{exceptionDelayMs}ms后重试。",
                            ...
                            if (!retryOffSucceeded)
                            {
                                var offFailure = new StartupPositioningResult
                                {
                                    ...
                                    Code = "StartupPositioningOutputOffCommandFailed",
                                    Reason =
                                        "OutputOffCommandFailed：启动定位重试前无法执行安全断电。" +
                                        $" Original={result.Code}/{result.Reason}",
                                    PeakCurrentA = result.PeakCurrentA,
                                    LastSlopeAperMs = result.LastSlopeAperMs,
                            ...
                            PublishChannelRuntimeState(
                                ...
                                $"启动定位软件瞬态未通过，第{attempt}次自愈，{delayMs}ms后重试。",
                                affectedChannels: enabled,
                                correlationId: runId,
                                allowTerminalReset: true);
                            _log.Warn(
