   ...
   public class ClsXmlOperation
    {
        ...
        public static string GetDaqAICurrentChannelMapping(string filePath, string DevName, out ConcurrentDictionary<string, int> channelMapping)
        {
                ...
                foreach (XmlNode recordsNode in recordsNodes)
                {
                    ...
                    if (enabledNode != null && enabledNode.InnerText.Trim() == "1")
                    {
                        ...
                        if (physicalChannelNode != null && paramNameNode != null && paramTypeNode != null)
                        {
                            ...
                            {
                                recordList.Add((physicalChannel, paramName));
                            }
                        }
                    }
                }
        ...
        public static string GetDaqAITorqueChannelMapping(string filePath, string DevName, out ConcurrentDictionary<string, int> channelMapping)
        {
            channelMapping = new ConcurrentDictionary<string, int>();
            try
            {
                ...
                foreach (XmlNode recordsNode in recordsNodes)
                {
                    ...
                    if (enabledNode != null && enabledNode.InnerText.Trim() == "1")
                    {
                        ...
                        if (physicalChannelNode != null && paramNameNode != null && paramTypeNode != null)
                        {
                            ...
                            // 增加参数类型为电流的限制
                            if (physicalChannel.Contains(DevName) && paramType == "扭矩")
                            {
                                recordList.Add((physicalChannel, paramName));
                            }
                        }
        ...
        public static string GetDaqAIPresssureChannelMapping(string filePath, string DevName, out ConcurrentDictionary<string, int> channelMapping)
        {
            channelMapping = new ConcurrentDictionary<string, int>();
            try
            {
                ...
                foreach (XmlNode recordsNode in recordsNodes)
                {
                    ...
                    if (enabledNode != null && enabledNode.InnerText.Trim() == "1")
                    {
                        ...
                        if (physicalChannelNode != null && paramNameNode != null && paramTypeNode != null)
                        {
                            ...
                            // 增加参数类型为电流的限制
                            if (physicalChannel.Contains(DevName) && paramType == "压力")
                            {
                                recordList.Add((physicalChannel, paramName));
                            }
                        }
        ...
        public static string GetDaqAIDistanceChannelMapping(string filePath, string DevName, out ConcurrentDictionary<string, int> channelMapping)
        {
            channelMapping = new ConcurrentDictionary<string, int>();
            try
            {
                ...
                foreach (XmlNode recordsNode in recordsNodes)
                {
                    ...
                    if (enabledNode != null && enabledNode.InnerText.Trim() == "1")
                    {
                        ...
                        if (physicalChannelNode != null && paramNameNode != null && paramTypeNode != null)
                        {
                            ...
                            // 增加参数类型为电流的限制
                            if (physicalChannel.Contains(DevName) && paramType == "距离")
                            {
                                recordList.Add((physicalChannel, paramName));
                            }
                        }
        ...
        public static string GetDaqScaleMapping(string filePath, string DevName, out ConcurrentDictionary<string, double> scaleMapping)
        {
            ...
            try
            {
                ...
                var enabledRecords = doc.Descendants("Records")
                    .Where(r => (string)r.Element("是否启用") == "1") // 过滤有效参数
                    .Select(r => new
                    {
        ...
        public static string GetDaqOffsetMapping(string filePath, string DevName, out ConcurrentDictionary<string, double> offsetMapping)
        {
            ...
            try
            {
                XDocument doc = XDocument.Load(filePath);
                var enabledRecords = doc.Descendants("Records")
                    .Where(r => (string)r.Element("是否启用") == "1" &&
                                (string)r.Element("参数名") != "--") // 过滤有效参数
                    .Select(r => new
                    {
                        Param = (string)r.Element("参数名"),
