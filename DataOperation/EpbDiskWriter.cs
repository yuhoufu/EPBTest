...
namespace DataOperation
{
    ...
    public sealed class EpbDiskWriter : IDisposable
    {
        ...
        private sealed class EpbState
        {
            ...
            public int StopAfterK = 0;             // 仅当 StopTrigger=AfterKMoreCycles
        }
        ...
        public EpbDiskWriter(DataRetentionPolicy policy)
        {
            ...
            for (int ch = 1; ch <= EPB_COUNT; ch++)
            {
                ...
                _mmfs[ch] = MemoryMappedFile.CreateFromFile(path, FileMode.Open, mapName: $"EPB{ch}_MMF", capacity: _fileBytes);
        ...
        /// </summary>
        public void SetStopRetentionOption(int epbId, int keepLatestN = 10, string action = "archive", StopTrigger trigger = StopTrigger.EndOfCurrentCycle, int afterK = 0)
        {
            var s = GetState(epbId);
            lock (s.Gate)
            {
        ...
        /// <summary>
        /// 立即对某通道执行“最新 N 圈”保留。
        /// action: delete（删索引）/ archive（导出 CSV 后删索引）。
        /// </summary>
        public void PersistLatestCyclesNow(int epbId, int keepLatestN = 10, string action = "archive")
        {
            ...
            if (action == "archive")
            {
                ...
                {
                    var csv = Path.Combine(dir, $"EPB{epbId}_Cycle_{cy.CycleNumber:D6}.csv");
        ...
        public void ApplyRetentionPolicy()
        {
            ...
            for (int epb = 1; epb <= EPB_COUNT; epb++)
        ...
        public void ExportCycleToCsv(int epbId, CycleInfo cycle, string csvPath)
        {
            ...
            using (var sw = new StreamWriter(csvPath, false))
            {
                ...
                for (int i = 0; i < cycle.SampleCount; i++)
                {
                    var idx = (cycle.StartRecordIndex + i) % s.CapacityRecords;
                    var rec = ReadRecord(v, idx * SampleRecord.Size);
                    if (rec.CycleNumber <= 0) continue; // 容错：如果被覆盖或脏，跳过
        ...
        /// <summary>
        /// 导出 Free-Run（Cycle=0）最近窗口（按样本数倒序回溯）。
        /// </summary>
        public void ExportFreeRunBySamples(int epbId, int sampleCount, string csvPath)
        {
            ...
            using (var sw = new StreamWriter(csvPath, false))
            {
                ...
                long n = 0;
                for (long back = 1; back <= s.CapacityRecords && n < sampleCount; back++)
                {
                    var written = s.TotalWritten;
                    if (written == 0) break;

                    var idx = (written - back) % s.CapacityRecords;
                    var rec = ReadRecord(v, idx * SampleRecord.Size);
        ...
        private string GetDatPath(int epbId)
            => Path.Combine(_rootDir, $"EPB{epbId}_sliding.dat");
        ...
        private static void EnsureFixedSizeFile(string path, long sizeBytes)
        {
            ...
            if (File.Exists(path))
            {
                ...
                if (fs.Length != sizeBytes) { fs.SetLength(sizeBytes); fs.Flush(true); }
                return;
            }
 ...
 ORDER BY cycle_number ASC";
    ...
    public sealed class DiskWriterRecorderAdapter : IEpbCycleRecorder
    {
        ...
        public void FlushRecent(int epbId, int lastNCycles)
            => _writer.PersistLatestCyclesNow(epbId, Math.Max(1, lastNCycles), action: "archive");
