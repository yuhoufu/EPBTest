...
| 运行失活看门狗 | 默认每 `2000 ms` 巡检；Timer 已停、已故障，或超过 `max(30 s, 2×周期)` 无循环心跳时启动安全自恢复，不再只停留在 `SystemFault` | `cd395cf`、`35ad0a2` |
...
提交 `35ad0a2` 把失活看门狗从“发现后停机”完善为“发现后安全重建并续测”。处理顺序固定为：
...
| 整改提交 | `cd395cf`、`ce3ce68`、`35ad0a2` | 追溯 Timer/运行态一致性、Timer 自动重建续测及程控电源超时 Task 修复 |
