...
| 运行失活看门狗 | 默认每 `2000 ms` 巡检；Timer 已停、已故障，或超过 `max(30 s, 2×周期)` 无循环心跳时启动安全自恢复，不再只停留在 `SystemFault` | `cd395cf`、`35ad0a2`、`cf9b33a` |
...
### 6.7 V2.11.0.15 Timer 异常自动续测

提交 `35ad0a2`、`cf9b33a` 把失活看门狗从“发现后停机”完善为“发现后安全重建并续测”，并接入共享液压组恢复所有权协调。处理顺序固定为：

1. 通道从 `Running/WarningRunning` 切换为 `Recovering`，不先写入不可自动解除的 `SystemFault`；
2. 暂停失活 Timer、取消当前圈、从液压参与者集合移除，并把当前未完成圈按软件恢复圈作废，不计正式完成数；
3. 使用高优先级 DO 确认电机断电；断电命令失败时保持 `Recovering` 并持续重试，确认断电前禁止自启；
4. 释放液压，重新执行 DAQ、电源和机械释放预检；
5. 移除旧 Timer，创建新 Timer，从当前压力组的公共未来完整节律槽重新加入；
6. 重建失败按 `1 s、2 s、5 s、10 s、30 s` 退避，之后保持每 `30 s` 重试，没有“重试若干次后永久停机”的软件门槛；
7. 人工停止/暂停、批次结束、项目禁用或硬件报警锁存一旦成立，自动恢复资格立即失效，不会误拉起卡钳。

Timer 恢复与液压、电源、DAQ 恢复共用现有优先级协调器。同一液压组同一时刻只有一个恢复所有者；DAQ/电源等更高优先级恢复可以取消 Timer 恢复，Timer 释放所有权后进入下一次退避，禁止两个恢复路径并发执行机械释放或重建节律。

自动重建适用于 Timer `Stopped/Completed/Faulted`、心跳陈旧，以及没有明确归属原因的异常 `PausePending/Paused`。以下暂停由原所有者处理，不启动重复恢复：批次/单通道人工优雅暂停、DAQ 恢复、液压恢复、电源恢复、正式圈软件恢复及电源组紧急关闭。

卡钳硬件故障边界保持不变。当前明确不可自动恢复并可持久禁用通道的故障码包括 `ForwardLoadRiseNotStarted`、`OpenCircuitOrOutputFault`、`ForwardLowPlateauConfirmed`、`ForwardPeakOvershoot2AConfirmed`；启动定位过流只有在卡钳电流与新鲜程控电源证据同时成立时才确认硬件故障。`OutputCommandFailed`、`TerminalOffCommandFailed`、`SoftwareRecoveryOffFailed`、`OffCurrentNotCleared` 属于输出控制链故障，保持安全断电重试，不再冒充卡钳硬件故障。明确卡钳硬件故障进入 `AlarmStopped` 或 `Enabled=false` 后，所有自动恢复门禁均拒绝自启。

### 6.8 V2.11.0.15 全停机路径自动续测边界

提交 `dbca02f` 对 Timer 之外的停机路径完成统一整改：

| 停止来源 | V2.11.0.15 行为 |
|---|---|
| DAQ 通信、任务或设备暂时离线 | 整组安全断电，保持 `Recovering`；持续重建和探测，设备恢复后按完整未来槽续测 |
| 液压同步、压力证据或释放超时 | 释放当前代次并持续组级复核；超过单次硬期限后执行 Stop→Start 等价清场，失败继续退避 |
| 程控电源通信、保护触发、意外 OFF 或低压 | 禁止动作并持续执行关闭、重连、参数回读和许可复核；保护解除后自动续测 |
| DO/输出关闭命令失败 | 保持断电重试和电源组恢复，不删除运行意图；可靠断电、机械释放和电源许可均通过后才续测 |
| Timer、正式圈控制或落盘异常 | 当前圈作废且不计数，重建 Timer/运行对象并从未来完整槽续测 |
| 可恢复卡钳控制故障 | 报警证据封存后持续执行定位与两圈资格复核，失败无限退避，成功自动续测 |
| 明确卡钳硬件故障 | 保持 `AlarmStopped`；四类客户故障同时写入 `Enabled=false`，禁止无人值守拉起 |
| 人工停止/暂停或项目禁用 | 立即撤销自动恢复资格，任何组级恢复不得重新接纳该通道 |

组级清场现在会过滤 `AlarmStopped`、报警锁存、`Enabled=false`、人工停止和人工暂停通道。基础设施恢复只拥有解除旧 `SystemFault` 的权限，不能解除卡钳 `AlarmStopped/InterlockStopped`，从状态机层面防止同组恢复误拉起硬件故障卡钳。
...
6. 注入 Timer `Faulted/Stopped` 或超过心跳阈值后，界面必须先显示 `Recovering`，安全边界完成后自动回到 `Running`；
7. 首次重建故意失败时，必须按退避策略持续重试，故障解除后无需人工点击即可创建新 Timer 并继续计数；
8. 注入人工暂停、批次暂停、`AlarmStopped`、`InterlockStopped` 和 `Enabled=false`，验证均不会被 Timer 自恢复拉起；
9. 恢复前的中断圈必须标记为软件恢复作废圈，恢复后的第一个正式圈不得重复累计。
...
6. 运行中拔掉并重新接入 DAQ，验证离线期间保持安全断电，重新接入后无需人工点击即可续测；
7. 触发并解除程控电源保护，验证保护未解除前不动作，解除并通过回读后自动续测；
8. 注入液压释放超时和首次组级清场失败，验证不会永久停在 `SystemFault`，后续成功后自动重入；
9. 注入 DO 关闭失败，验证持续发送安全关闭且不删除运行意图；关闭恢复后完成机械释放再续测；
10. 同组另一路置为明确卡钳报警、人工停止或禁用，验证外部设备恢复只拉起合格通道。
...
- 任一 Running 通道超过 30 秒无有效心跳时，界面自动转为 `Recovering`，安全预检通过后重建 Timer 并继续测试；
...
- 明确卡钳硬件报警、人工停止/暂停和项目禁用绝不自动拉起；非硬件 Timer 异常解除后无需人工干预即可续测。
- DAQ、液压、程控电源和输出控制链故障即使持续超过一次恢复硬期限，也不得永久停机；故障解除后必须自动完成安全预检并继续测试。
...
- 新增“非人工Timer异常允许自动重建续测”测试通过，覆盖 `Faulted`、无归属暂停、硬件报警锁存、人工暂停、批次暂停和项目禁用门禁；
- 新增“Timer自恢复持续退避且不停止重试”测试通过，确认长期退避上限为 30 秒且没有软件永久停机阈值；
- 新增“只有卡钳通道硬件故障允许锁存停机”测试通过；
- 新增“外部设备恢复不得拉起报警禁用或人工暂停通道”测试通过；
- 新增“可恢复卡钳故障持续自启且硬件锁存禁止自启”测试通过；
- 新增“基础设施恢复只解除系统故障不解除卡钳报警”测试通过；
...
| 整改提交 | `cd395cf`、`ce3ce68`、`35ad0a2`、`cf9b33a`、`dbca02f` | 追溯 Timer/运行态一致性、Timer 自动重建续测、全停机路径自动恢复、恢复所有权协调及程控电源超时 Task 修复 |

---

本报告没有把缺少直接证据的“具体 Pause 发起分支”写成确定根因。V2.11.0.15 已落地：**Timer/运行态同步、运行失活检测、DAQ/液压/电源/输出控制链与可恢复卡钳故障的持续安全自恢复、明确卡钳硬件/人工停机防误自启、SCPI 超时 Task 故障观察与传输隔离**；剩余工作是按第 8、9 节完成真实设备故障注入验收。
