...
3. 使用高优先级 DO 确认电机断电；断电命令失败立即升级为电源组硬件联锁，禁止继续自启；
...
卡钳硬件故障边界保持不变。当前明确不可自动恢复并可持久禁用通道的故障码包括 `ForwardLoadRiseNotStarted`、`OpenCircuitOrOutputFault`、`ForwardLowPlateauConfirmed`、`ForwardPeakOvershoot2AConfirmed`；`OutputCommandFailed`、`TerminalOffCommandFailed`、`SoftwareRecoveryOffFailed`、`OffCurrentNotCleared` 等断电不可确认故障继续按硬件锁存处理。上述状态进入 `AlarmStopped/InterlockStopped` 或 `Enabled=false` 后，Timer 恢复门禁拒绝自启。
...
| 整改提交 | `cd395cf`、`ce3ce68`、`35ad0a2`、`cf9b33a` | 追溯 Timer/运行态一致性、Timer 自动重建续测、恢复所有权协调及程控电源超时 Task 修复 |

---

本报告没有把缺少直接证据的“具体 Pause 发起分支”写成确定根因。V2.11.0.15 已落地：**Timer/运行态同步、运行失活检测、非硬件 Timer 异常安全重建并自动续测、硬件/人工停机防误自启、SCPI 超时 Task 故障观察与传输隔离**；剩余工作是按第 8、9 节完成真实设备故障注入验收。
