...
| 报警中断 | `alarm` | 是 | ClampTimeout、峰值过冲、IO 安全故障 |
...
旧查询中统计 RunCount 的语句保持：

```sql
status IN ('completed', 'alarm')
```
...
| SIM-05 | ClampTimeout | 只产生一次 alarm、一次计数和一次快照请求 | [ ] | — |
