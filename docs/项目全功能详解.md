...
3. 本次运行的报警锁存取得首个报警处理权，重复报警被去重；
4. 使用高优先级路径关闭该路 EPB 输出；
5. 停止该通道定时器和取消令牌；
6. 激活对应报警灯，并根据配置决定蜂鸣器；
7. 异步导出报警快照；
8. 校验当前报警圈同名 CSV/BIN 文件；
9. 证据完整时封为 `alarm`，否则封为 `failed` 并记录警告。

单通道启动和批量启动都会先重置该通道的报警停机锁存。因此，同一次
运行只处理首个报警，新一次运行又能正常响应首次报警。
...
`alarm` 是有文件证据的状态：报警通道目录中必须同时存在
`EPB<通道>_Cycle_<圈号>.csv` 和 `.bin`。该规则避免数据库出现无法回放、
无法核验的报警圈。
...
- Windows 内存映射分块写入；
- SQLite 保存圈级索引；
- 支持 running/completed/alarm/failed/canceled 状态；
- 支持 CSV/BIN 导出；
- 支持最近 N 圈保留策略和报警快照。
- 程序重启时从 SQLite 最后一圈恢复环形写游标；
- 导出前逐条校验圈号、样本序号和时间戳；
- CSV/BIN 使用临时文件成对写入并原子提交；
- 环形数据被覆盖时可从已有历史 BIN 快照恢复；
- 归档成功后才删除圈索引，失败圈保留并写 `export_errors.txt`。
...
二进制记录布局保持不变。CSV 在时间戳之后增加
`RelativeTimeSeconds`，完整表头为：

`Timestamp,RelativeTimeSeconds,Cycle,SampleIndex,EpbCurrent,GroupPressure`

相对时间以每个 CSV 的首条记录为 0 秒。兼容旧数据时，如果原始绝对
时间戳出现小幅回拨，相对时间会钳制为不小于上一条，确保回放时间轴单调。
...
打开已有项目时，写游标由 SQLite 中最新圈的
`start_position + sample_count` 恢复，再在实际访问时按容量取模。
逻辑位置保留累计写入语义，避免刚好写满一圈时被误判为从未写入。
...
| `status` | `running`、`completed`、`alarm`、`failed`、`canceled` |

`(epb_id, cycle_number)` 唯一。新会话从 `GetMaxCycleNumber + 1` 开始，不能复用旧圈号；`GetLastCycleNumber` 仅为兼容旧界面名称，实际返回成功圈数。
...
- 正常完成圈计数；
- 报警、失败、取消和尚未封圈的 `running` 均不计数；
- UI、XML 和数据库以完整成功圈为共同口径。
